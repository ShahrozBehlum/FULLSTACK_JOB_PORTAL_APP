...
const ApplyJob = () => {
  ...
  const { jobs, backendUrl, userData, userToken, fetchUserApplications, userApplications} = useContext(AppContext);
  ...
  const fetchJob = async () => {
    try {
      ...
      // console.log("Job data:", JSON.stringify(data.job, null, 2));
  ...
  const applyHandler = async () => {
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.message || "Already Applied For This Job")
      }
  ...
  setIsAlreadyApplied(Boolean(hasApplied));
};
  ...
  return JobData ? (
    <>
      ...
      <div className='container min-h-screen flex flex-col py-10 px-4 2xl:px-20 mx-auto'>
        <div className='bg-white text-black rouned-lg w-full'>
          ...
          <div className='flex flex-col lg:flex-row justify-between items-start'>
            <div className='w-full lg:w-2/3'>
              ...
              <button className='bg-blue-600 p-2.5 px-10 text-white rounded mt-10'>{isAlreadyApplied ? "Already Applied" : "Apply Now"}</button>
            </div>
